...
export default function Maintenance() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () =>
      api<{ data: any[]; pagination: any }>("/maintenance", {
            ...
            {isLoading ? (
              ...
              (data?.data ?? []).map((r: any) => (
                <tr key={r.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3 text-gray-500">{r.property?.name} #{r.unit?.unitNumber}</td>
                  <td className="px-4 py-3 text-gray-500">{r.tenant?.firstName} {r.tenant?.lastName}</td>
